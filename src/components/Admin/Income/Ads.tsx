...
export default function Ads() {
  ...
  const handleViewProduct = (productId: number) => {
    router.push(`/dashboard/income/ads/${productId}`);
  };
    ...
    <div className="min-h-screen">
      <div className="p-4 sm:p-6 flex flex-col gap-6">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => window.history.back()} className="px-2 py-2 rounded-3xl font-medium">Ads</button>
        </div>
        ...
        <div className="flex flex-col md:flex-row bg-white p-4 sm:p-5 md:py-8 gap-4 md:gap-5 rounded-lg">
          ...
          <Link href="/dashboard/income/ads/add-ads"
            onClick={handleApplyFilter}
            className="px-6 flex items-center gap-2 py-2.5 bg-[#FFE135] rounded-lg font-medium hover:bg-[#FFE135] transition-colors w-full md:w-auto"
          >
            <Plus size={20} /> Add Ads
          </Link>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[800px]">
              <thead className="bg-white border-b">
                <tr>
                  {['Business Name', 'Ads ID', 'Value', 'Created Date', 'Duration', 'Action'].map((h) => (
              ...
              <tbody className="divide-y divide-gray-100">
                {currentData.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 sm:px-6 py-3">7 days</td>
                    <td className="px-4 sm:px-6 py-3">
                      <div className="flex flex-wrap gap-2">
                        <button
                          onClick={() => handleViewProduct(product.id)}
                          className="px-4 py-1.5 text-sm rounded transition-colors bg-[#FFE135] text-gray-900 hover:bg-[#ffd700]"
                        >
                          View
                        </button>
                        <button
